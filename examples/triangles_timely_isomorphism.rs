...
//            let dK3dA = dABdA.intersect_only(vec![Box::new(forward.intersect_using(|x: &Vec<u32>| x[1],|x: &Vec<u32>| x[2]))]);
            let dK3dA = dG_init.extend(vec![Box::new(forward.extend_using(|x: &Vec<u32>| x[0], <_ as PartialOrd>::lt)),
            ...
            let dK3dB = dG_init.extend(vec![Box::new(forward.extend_using(|x: &Vec<u32>| x[0], <_ as PartialOrd>::le)),
            ...
            let dK3dC = dG_init.extend(vec![Box::new(reverse.extend_using(|x: &Vec<u32>| x[0], <_ as PartialOrd>::le)),
            ...
            //let cliques = dK3dC;
            //elapsed: 556.482253979s	total triangles at this process: 888141683
