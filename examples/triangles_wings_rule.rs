...
fn main () {
    ...
    timely::execute_from_args(std::env::args(), move |root| {
        ...
        let (mut input, probe, forward, reverse) = root.dataflow::<u32,_,_>(|builder| {
            ...
            let forward = IndexStream::from(|k| k as u64, &Vec::new().to_stream(builder), &dG, true);
            let reverse = IndexStream::from(|k| k as u64, &Vec::new().to_stream(builder), &dG.map(|((src,dst),wgt)| ((dst,src),wgt)), false);

            // dA(x,y) extends to z first through C(x,z) then B(y,z), both using forward indices.

//            let dABdA = dG_init.extend(vec![Box::new(forward.extend_using(|x: &Vec<u32>| x[0]))])
...
//            let dK3dA = dABdA.intersect_only(vec![Box::new(forward.intersect_using(|x: &Vec<u32>| x[1],|x: &Vec<u32>| x[2]))]);
            ...
            let dK3dB = dG_init.extend(vec![Box::new(forward.extend_using(|x: &Vec<u32>| x[0])),
                                            Box::new(reverse.extend_using(|x: &Vec<u32>| x[1]))])
            ...
            let dK3dC = dG_init.extend(vec![Box::new(reverse.extend_using(|x: &Vec<u32>| x[0])),
                                            Box::new(reverse.extend_using(|x: &Vec<u32>| x[1]))])
